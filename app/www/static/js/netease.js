...
async function refreshLoginStatus(showToastMsg = false) {
  try {
    ...
    if (json.success && json.logged_in) {
      ...
      localStorage.setItem('xiaosongshu_netease_user', JSON.stringify(user));
      ...
      localStorage.removeItem('xiaosongshu_netease_user');
...
export async function initNetease(onRefreshSongs) {
  ...
  try {
    const saved = localStorage.getItem('xiaosongshu_netease_user');
    if (saved) {
