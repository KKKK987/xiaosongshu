...
export function switchTab(tab) {
  ...
  ui.navPlaylists?.classList.remove('active');
  ...
  else if (tab === 'playlists') ui.navPlaylists?.classList.add('active');
  ...
  if (tab === 'playlists') {
    ...
    ui.viewQQMusic?.classList.remove('hidden');
  } else if (tab === 'upload') {
...
function loadTrackInfo(track) {
  ...
  document.title = `${track.title} - 小松鼠`;
...
export function bindUiControls() {
  ...
  window.addEventListener('storage', (e) => {
    if (e.key === 'xiaosongshu_favs' && e.newValue) {
      try {
  ...
  // 定位当前播放歌曲按钮
  if (ui.btnLocatePlaying) {
    ui.btnLocatePlaying.addEventListener('click', () => {
      locateCurrentPlayingSong();
    });
  }
}
...
async function loadPlaylists() {
  ...
  try {
    ...
    ui.playlistsContainer.innerHTML = '<div class="loading-text" style="padding: 4rem 0; opacity: 0.6;">加载失败</div>';
  }
}
...
function renderPlaylists(playlists) {
  ...
  if (playlists.length === 0) {
    ui.playlistsContainer.innerHTML = '<div class="loading-text" style="padding: 4rem 0; opacity: 0.6;">暂无歌单，点击右上角创建</div>';
    return;
  }
  ...
  playlists.forEach(pl => {
    ...
    wrapper.dataset.playlistId = pl.id;
    ...
    songsArea.className = 'playlist-songs-area hidden';
    songsArea.innerHTML = '<div class="loading-text" style="padding: 2rem 0; opacity: 0.6;">加载中...</div>';
...
// 切换歌单展开/收起
async function togglePlaylistExpand(playlistId, playlistName, wrapper, songsArea) {
...
function renderPlaylistSongsInline(songs, pendingSongs, playlistId, container) {
  ...
  if (pendingSongs && pendingSongs.length > 0) {
    pendingSongs.forEach((song, index) => {
      ...
      card.dataset.qqMid = song.qq_mid || '';
      card.dataset.neteaseId = song.netease_id || '';
      
      card.innerHTML = `
        <button class="card-fav-btn card-remove-btn" title="从歌单移除"><i class="fas fa-minus"></i></button>
        <div class="pending-badge"><i class="fas fa-download"></i> 待下载</div>
        <img src="${song.cover}" loading="lazy">
        <div class="card-info">
          <div class="title" title="${song.title}">${song.title}</div>
          <div class="artist">${song.artist}</div>
...
// 更新歌单歌曲数量显示
function updatePlaylistSongCount(container) {
  const wrapper = container.closest('.playlist-wrapper');
  if (wrapper) {
    const countEl = wrapper.querySelector('.playlist-count');
    if (countEl) {
      const remaining = container.querySelectorAll('.song-card').length;
      countEl.textContent = `${remaining} 首歌曲`;
    }
  }
  if (container.querySelectorAll('.song-card').length === 0) {
    container.innerHTML = '<div class="loading-text" style="padding: 2rem 0; opacity: 0.6;">歌单为空</div>';
  }
}
...
async function openPlaylistDetail(playlistId, playlistName) {
  ...
  if (wrapper) {
    ...
    if (songsArea && !wrapper.classList.contains('expanded')) {
      togglePlaylistExpand(playlistId, playlistName, wrapper, songsArea);
    }
  }
}
...
function renderPlaylistSongs(songs, playlistId) {
  if (!ui.playlistSongsContainer) return;
  
  if (songs.length === 0) {
    ui.playlistSongsContainer.innerHTML = '<div class="loading-text" style="grid-column: 1/-1; padding: 4rem 0; opacity: 0.6;">歌单为空</div>';
    return;
  }
  
  ui.playlistSongsContainer.innerHTML = '';
  const frag = document.createDocumentFragment();
  ...
  songs.forEach((song, index) => {
    ...
    card.querySelector('.card-fav-btn').addEventListener('click', async (e) => {
      ...
      try {
        ...
        if (res.success) {
          showToast('已从歌单移除');
          openPlaylistDetail(playlistId, ui.playlistDetailName?.textContent || '');
        } else {
          showToast(res.error || '移除失败');
        }
      } catch (err) {
  ...
  ui.playlistSongsContainer.appendChild(frag);
}

// 创建歌单
async function createPlaylist(name, callback) {
  if (!name || !name.trim()) {
    showToast('请输入歌单名称');
    return;
  }
  ...
  try {
    ...
    if (res.success) {
      showToast('歌单创建成功');
      if (callback) callback(res.playlist);
      loadPlaylists();
    } else {
...
function renamePlaylist(playlistId, currentName) {
  const newName = prompt('请输入新的歌单名称:', currentName);
  if (newName && newName.trim() && newName !== currentName) {
    api.playlists.rename(playlistId, newName.trim()).then(res => {
      if (res.success) {
        showToast('重命名成功');
        loadPlaylists();
      } else {
...
function deletePlaylist(playlistId, playlistName) {
  showConfirmDialog('删除歌单', `确定要删除歌单"${playlistName}"吗？此操作不可恢复。`, async () => {
    try {
      const res = await api.playlists.delete(playlistId);
      if (res.success) {
        showToast('歌单已删除');
        loadPlaylists();
      } else {
...
async function showAddToPlaylistModal(song) {
  ...
  if (ui.addToPlaylistModal) ui.addToPlaylistModal.classList.add('active');
  if (ui.addToPlaylistList) ui.addToPlaylistList.innerHTML = '<div class="loading-text" style="padding: 2rem 0; opacity: 0.6;">加载中...</div>';
  
  try {
    const res = await api.playlists.list();
    if (res.success) {
      renderPlaylistSelectList(res.playlists || []);
    } else {
      ui.addToPlaylistList.innerHTML = `<div class="loading-text" style="padding: 2rem 0; opacity: 0.6;">${res.error || '加载失败'}</div>`;
    }
  } catch (e) {
    ui.addToPlaylistList.innerHTML = '<div class="loading-text" style="padding: 2rem 0; opacity: 0.6;">加载失败</div>';
  }
}

// 渲染歌单选择列表
function renderPlaylistSelectList(playlists) {
  if (!ui.addToPlaylistList) return;
  
  if (playlists.length === 0) {
    ui.addToPlaylistList.innerHTML = '<div class="loading-text" style="padding: 2rem 0; opacity: 0.6;">暂无歌单，请先创建</div>';
    return;
  }
  
  ui.addToPlaylistList.innerHTML = '';
  ...
  playlists.forEach(pl => {
    ...
    item.innerHTML = `
      ...
      <div class="info">
        ...
        <div class="count">${pl.song_count} 首歌曲</div>
      </div>
    `;
    
    item.addEventListener('click', () => addSongToPlaylist(pl.id, pl.name));
    ui.addToPlaylistList.appendChild(item);
  });
}
...
async function addSongToPlaylist(playlistId, playlistName) {
  ...
  try {
    const res = await api.playlists.addSong(playlistId, pendingSongForPlaylist.id);
    if (res.success) {
      showToast(`已添加到"${playlistName}"`);
      ui.addToPlaylistModal?.classList.remove('active');
...
function initPlaylistsUI() {
  // 导航点击
  ui.navPlaylists?.addEventListener('click', () => switchTab('playlists'));
  
  // 新建歌单按钮
  ui.btnCreatePlaylist?.addEventListener('click', () => {
    ui.createPlaylistModal?.classList.add('active');
    if (ui.newPlaylistNameInput) ui.newPlaylistNameInput.value = '';
  });
  
  // 弹窗中的新建歌单按钮
  ui.btnCreatePlaylistInModal?.addEventListener('click', () => {
    ui.addToPlaylistModal?.classList.remove('active');
    ui.createPlaylistModal?.classList.add('active');
    if (ui.newPlaylistNameInput) ui.newPlaylistNameInput.value = '';
  });
  ...
  // 关闭添加到歌单弹窗
  ui.closeAddToPlaylistModal?.addEventListener('click', () => {
    ui.addToPlaylistModal?.classList.remove('active');
    pendingSongForPlaylist = null;
  });
  
  // 关闭新建歌单弹窗
  ui.closeCreatePlaylistModal?.addEventListener('click', () => {
    ui.createPlaylistModal?.classList.remove('active');
  });
  
  // 确认创建歌单
  ui.confirmCreatePlaylist?.addEventListener('click', () => {
    const name = ui.newPlaylistNameInput?.value?.trim();
    if (name) {
      createPlaylist(name, (newPlaylist) => {
        ui.createPlaylistModal?.classList.remove('active');
        // 如果是从添加歌曲弹窗来的，自动添加歌曲到新歌单
        if (pendingSongForPlaylist && newPlaylist) {
          addSongToPlaylist(newPlaylist.id, newPlaylist.name);
        }
      });
    } else {
      showToast('请输入歌单名称');
  ...
  // 回车创建歌单
  ui.newPlaylistNameInput?.addEventListener('keydown', (e) => {
    if (e.key === 'Enter') {
      ui.confirmCreatePlaylist?.click();
    }
  });
  
  // 点击弹窗背景关闭
  ui.addToPlaylistModal?.addEventListener('click', (e) => {
    if (e.target === ui.addToPlaylistModal) {
      ui.addToPlaylistModal.classList.remove('active');
      pendingSongForPlaylist = null;
    }
  });
  
  ui.createPlaylistModal?.addEventListener('click', (e) => {
    if (e.target === ui.createPlaylistModal) {
      ui.createPlaylistModal.classList.remove('active');
    }
  });
}
...
// 显示下载来源选择弹窗
function showDownloadSourceDialog(song) {
  const overlay = document.createElement('div');
  overlay.className = 'custom-overlay centered active';
  overlay.id = 'download-source-modal';
  
  const searchQuery = `${song.artist} ${song.title}`.trim();
  
  overlay.innerHTML = `
    <div class="qr-box glass-panel" style="min-width: 320px; max-width: 400px;">
      <h3 style="margin-bottom: 1rem;"><i class="fas fa-download"></i> 下载歌曲</h3>
      <div style="margin-bottom: 1rem;">
        <div style="font-size: 1rem; font-weight: 600; margin-bottom: 0.3rem;">${song.title}</div>
        <div style="font-size: 0.9rem; color: var(--text-sub);">${song.artist}</div>
      </div>
      <div style="background: rgba(255,255,255,0.05); border-radius: 0.5rem; padding: 0.8rem; margin-bottom: 1rem; font-size: 0.85rem; color: var(--text-sub);">
        本地库中没有这首歌曲，请选择下载来源
      </div>
      <div style="display: flex; flex-direction: column; gap: 0.5rem;">
        <button id="download-from-qq" class="btn-primary" style="width: 100%; padding: 0.8rem; background: linear-gradient(135deg, #12b7f5, #1db954);">
          <i class="fab fa-qq"></i> 从 QQ 音乐下载
        </button>
        <button id="download-from-netease" class="btn-primary" style="width: 100%; padding: 0.8rem; background: linear-gradient(135deg, #c20c0c, #e60026);">
          <i class="fas fa-cloud"></i> 从网易云音乐下载
        </button>
        <button id="download-cancel" class="btn-normal" style="width: 100%; padding: 0.6rem; opacity: 0.7;">
          取消
        </button>
      </div>
    </div>
  ...
  // QQ 音乐下载
  overlay.querySelector('#download-from-qq').addEventListener('click', () => {
    overlay.remove();
    // 切换到 QQ 音乐标签页并搜索
    switchToTabAndSearch('qqmusic', searchQuery);
  });
  
  // 网易云音乐下载
  overlay.querySelector('#download-from-netease').addEventListener('click', () => {
    overlay.remove();
    // 切换到网易云标签页并搜索
    switchToTabAndSearch('netease', searchQuery);
  });
...
export { loadPlaylists, showAddToPlaylistModal };
