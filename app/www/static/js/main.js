...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  if (ui.navQQMusic) ui.navQQMusic.addEventListener('click', () => { switchTab('qqmusic'); });
  ...
  function initSettings() {
    ...
    // Load initial
    const saved = localStorage.getItem('xiaosongshu_ui_scale');
    if (saved) {
    ...
    ui.scaleInput.addEventListener('change', (e) => {
      ...
      localStorage.setItem('xiaosongshu_ui_scale', e.target.value);
    });

    ui.scaleReset?.addEventListener('click', () => {
      localStorage.removeItem('xiaosongshu_ui_scale');
